...
# FIGURE 5A - Genetic signatures of microglial subtypes (PMID 31276795 - Dachet et al). Are these populations represented in CS52 microglia OLD or NEW? Make barplots and run stats
```{r, echo=FALSE}
barplotdata <- as.data.frame(topTags(QLFtestX2, n=sum(decideTestsobjX2[c(1,2,3),]), sort.by="PValue"))

MGtype1 <- scan(text="A2M
ABCB1
APOD
C7
CALCRL
...
VPS13C
WWTR1", what="")
MGtype1 <- dput(MGtype1)

MGtype2 <- scan(text="ABCA8	TMEM123
ADIPOR2	TOP2B
AEBP1	TREM2
ALDH1A1	TRIP6
ANTXR1	VCAM1
...
SPARC	
SULT1C4", what="")
MGtype2 <- dput(MGtype2)

#The gene lists above can be saved as an individual .rds files, and then loaded into any R script for analysis. 
saveRDS(MGtype1,"./PMID 31276795 MG subtype/MGtype1.rds")
saveRDS(MGtype2,"./PMID 31276795 MG subtype/MGtype2.rds")

MGtype1 <- readRDS(file="./PMID 31276795 MG subtype/MGtype1.rds")
MGtype2 <- readRDS(file="./PMID 31276795 MG subtype/MGtype2.rds")

#Can PMID 31276795 Dachet MG subtype 1 or MG subtype 2 signatures be found in either of our cultures (OLD or NEW)? Make barplots of full gene signature for each MG subtype

par(mar=c(4,8,0.5,5)+0.1, xpd=TRUE, oma=c(0,0,0,0)) # mar is a numerical vector of the form c(bottom, left, top, right) which gives the number of lines of margin to be specified on the four sides of the plot. The default is c(5, 4, 4, 2) + 0.1.
   MGtype1_logFC <- barplotdata[barplotdata$Gene_Symbol %in% MGtype1,c(2,4,8)]
MGtype1_barplotALL <- barplot(MGtype1_logFC$logFC, names.arg=MGtype1_logFC$Gene_Symbol, col=red_upreg_blue_downreg_blk_ns(MGtype1_logFC), main="Expression of MG subtype 1 (PMID 31276795) in New vs Old protocol", xlab="Log Fold Change (logFC)", border="white", las=1, cex.axis=2, cex.names=1, cex.lab=1.5, horiz = TRUE, xlim = c(-10,10))
MGtype1_barplotALL

 MGtype2_logFC <- barplotdata[barplotdata$Gene_Symbol %in% MGtype2,c(2,4,8)]
par(mar=c(4,4,0.5,5)+0.1, xpd=TRUE, oma=c(0,0,0,0)) # mar is a numerical vector of the form c(bottom, left, top, right) which gives the number of lines of margin to be specified on the four sides of the plot. The default is c(5, 4, 4, 2) + 0.1.
MGtype2_barplotALL <- barplot(MGtype2_logFC$logFC, names.arg=MGtype2_logFC$Gene_Symbol, col=red_upreg_blue_downreg_blk_ns(MGtype2_logFC), main="Expression of MG subtype 2 (PMID 31276795) in New vs Old protocol", xlab="Log Fold Change (logFC)", border="white", las=1, cex.axis=1.2, cex.names=0.8, cex.lab=1.5, horiz = TRUE, xlim = c(-10,10), width=rep(60,times=nrow(MGtype2_logFC))) MGtype2_barplotALL

#Is the overlap between the OLD vs NEW DEGs and Dachet MG types significant? Use the Fishers exact test to check this
...
#Is MGtype1 significantly enriched in OLD microglia? Compare MGtype1 to all downreg DEGs (downreg DEGs in OLD vs NEW analysis = genes increased in OLD protocol)
overlap_MGtype1.OLD <- newGeneOverlap(MGtype1,downreg_genes,total)
overlap_MGtype1.OLD <- testGeneOverlap(overlap_MGtype1.OLD)
overlap_MGtype1.OLD
overlap_MGtype1.OLD_allstats <- print(overlap_MGtype1.OLD)
paste("================================================")
#Is MGtype1 significantly enriched in NEW microglia? Compare MGtype1 to all upreg DEGs (upreg DEGs in OLD vs NEW analysis = genes increased in NEW protocol)
overlap_MGtype1.NEW <- newGeneOverlap(MGtype1,upreg_genes,total)
overlap_MGtype1.NEW <- testGeneOverlap(overlap_MGtype1.NEW)
overlap_MGtype1.NEW
overlap_MGtype1.NEW_allstats <- print(overlap_MGtype1.NEW)
paste("================================================")

#Is MGtype2 significantly enriched in OLD microglia? Compare MGtype1 to all downreg DEGs (downreg DEGs in OLD vs NEW analysis = genes increased in OLD protocol)
overlap_MGtype2.OLD <- newGeneOverlap(MGtype2,downreg_genes,total)
overlap_MGtype2.OLD <- testGeneOverlap(overlap_MGtype2.OLD)
overlap_MGtype2.OLD
overlap_MGtype2.OLD_allstats <- print(overlap_MGtype2.OLD)
paste("================================================")
#Is MGtype2 significantly enriched in NEW microglia? Compare MGtype1 to all upreg DEGs (upreg DEGs in OLD vs NEW analysis = genes increased in NEW protocol)
overlap_MGtype2.NEW <- newGeneOverlap(MGtype2,upreg_genes,total)
overlap_MGtype2.NEW <- testGeneOverlap(overlap_MGtype2.NEW)
overlap_MGtype2.NEW
overlap_MGtype2.NEW_allstats <- print(overlap_MGtype2.NEW)
```

# FIGURE 5B - Keren-Shaul et al identifies disease-associated microglia (DAM) and provided a list of DEGs between DAM and homeostatic microglia in ALS spinal cord. From the excel sheet of Keren-Shaul's Table S6 (~34k genes), I deleted all the rows with FC=0 (leaving 15kgenes) and all rows with Pvalue>0.05 (leaving 5k genes). DAM DEGs as those with FC>0 (~1000 genes) and homeostatic genes as those with FC<0 (~4k genes).
```{r}
DAMgenes_DF <- read.table(file="./Differentiation Protocol comparison/Keren-Shaul et al_Table S6_DAM genes only.csv", header=TRUE, sep=",")
DAMgenes_DF <- DAMgenes_DF[order(DAMgenes_DF$X.log10.p.value.,decreasing = T),]
head(DAMgenes_DF)

DAMgenes <- DAMgenes_DF$NAME
head(DAMgenes)

# Making barplots
barplotdata <- as.data.frame(topTags(QLFtestX2, n=sum(decideTestsobjX2[c(1,2,3),]), sort.by="PValue"))

par(mar=c(4,8,0.5,5)+0.1, xpd=TRUE, mfrow=c(1,1)) # mar is a numerical vector of the form c(bottom, left, top, right) which gives the number of lines of margin to be specified on the four sides of the plot. The default is c(5, 4, 4, 2) + 0.1.
DAMgenes_logFC <- barplotdata[barplotdata$Gene_Symbol %in% DAMgenes[1:62],c(2,4,8)]
DAMgenes_barplot <- barplot(DAMgenes_logFC$logFC, names.arg=DAMgenes_logFC$Gene_Symbol, main="Expression of top 50 DAM genes in New vs Old protocol", col=red_upreg_blue_downreg_blk_ns(DAMgenes_logFC), border="NA", xlab="Log Fold Change (logFC)", las=1, cex.axis=2, cex.names=1, cex.lab=1.5,horiz = TRUE, xlim = c(-10,10),width=rep(60,times=nrow(DAMgenes_logFC)))
  
  #Is the DAM signature significantly enriched in OLD microglia? Compare to all downreg DEGs (downreg DEGs in OLD vs NEW analysis = genes increased in OLD protocol)
paste("Is DAM significantly enriched in OLD microglia?")
overlap_DAM.OLD <- newGeneOverlap(DAMgenes,downreg_genes,total)
overlap_DAM.OLD <- testGeneOverlap(overlap_DAM.OLD)
overlap_DAM.OLD
overlap_DAM.OLD_allstats <- print(overlap_DAM.OLD)
paste("================================================")
#Is DAM significantly enriched in NEW microglia? Compare to all upreg DEGs (upreg DEGs in OLD vs NEW analysis = genes increased in NEW protocol)
paste("Is DAM significantly enriched in NEW microglia?")
overlap_DAM.NEW <- newGeneOverlap(DAMgenes,upreg_genes,total)
overlap_DAM.NEW <- testGeneOverlap(overlap_DAM.NEW)
overlap_DAM.NEW
overlap_DAM.NEW_allstats <- print(overlap_DAM.NEW)
paste("================================================")
paste("================================================")
